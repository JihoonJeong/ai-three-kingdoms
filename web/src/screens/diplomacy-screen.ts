...
export class DiplomacyScreen {
  ...
  render(container: HTMLElement, state: GameState): void {
    ...
    wrap.appendChild(h('div', { className: 'panel-header' }, t('외교 관계')));
    ...
    allPanel.innerHTML = `<div class="panel-header">${t('전체 세력 관계')}</div>`;
    ...
    for (const rel of state.diplomacy.relations) {
      ...
      row.innerHTML = `
        <span><span class="badge ${infoA?.badgeClass ?? ''}">${t(rel.factionA)}</span>
        ↔ <span class="badge ${infoB?.badgeClass ?? ''}">${t(rel.factionB)}</span></span>
        <span>${t(rel.relation)} (${rel.value}) ${rel.isAlliance ? `⚔ ${t('동맹')}` : ''}</span>`;
  ...
  private renderFactionCard(factionId: FactionId, rel: DiplomacyRelation | null, state: GameState): HTMLElement {
    ...
    header.append(
      h('span', { className: `badge ${info?.badgeClass ?? ''}` }, t(info?.name ?? factionId)),
    );
    if (rel?.isAlliance) {
      header.append(' ', h('span', { className: 'badge badge-liu' }, t('동맹')));
    }
    ...
    card.appendChild(createGauge(value, 100, gaugeColor, `${t(level)}: ${value}/100`));
    ...
    if (leader) {
      ...
      leaderRow.textContent = `${t('군주:')} ${t(leader.name)} (${t(leader.courtesyName)})`;
    ...
    cityRow.textContent = `${t('도시')}: ${fCities.map(c => t(c.name)).join(', ') || t('없음')}`;
    ...
    if (rel && rel.events.length > 0) {
      ...
      evtRow.textContent = t('최근:') + ' ' + rel.events.slice(-2).map(e => t(e)).join(' / ');
    ...
    const actions = [
      { label: t('사신 파견'), action: 'send_envoy', params: { target: factionId, purpose: '우호 증진' } },
      { label: t('선물'), action: 'gift', params: { target: factionId, amount: 1000 } },
      { label: t('위협'), action: 'threaten', params: { target: factionId } },
    ];
